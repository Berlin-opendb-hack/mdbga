...
import (
	...
	"github.com/Berlin-opendb-hack/mdbga/tool/cli"
...
func main() {
	...
	app := &cobra.Command{
		Use:   "mdbga-cli",
		Short: `CLI client for the mdbga service`,
	}
	...
	app.PersistentFlags().StringVarP(&c.Host, "host", "H", "localhost:8881", "API hostname")
	...
	tokenSigner := newTokenSigner(key, format)

	// Initialize API client
	c.SetTokenSigner(tokenSigner)
	c.UserAgent = "mdbga-cli/0"
...
// newTokenSigner returns the request signer used for authenticating
// against the token security scheme.
func newTokenSigner(key, format string) goaclient.Signer {
	return &goaclient.APIKeySigner{
		SignQuery: false,
		KeyName:   "Authorisation",
		KeyValue:  key,
		Format:    format,
	}
